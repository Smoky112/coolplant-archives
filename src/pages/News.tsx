...
import { useSearchParams } from "react-router-dom"; // Logica aggiornata: Import per i parametri URL
import { Calendar, User, Tag, ExternalLink, X, Mail, Check, AlertTriangle } from "lucide-react";
...
import { edenTried } from "@/lib/terminalState"; // Assicurati che il path sia corretto
...
interface NewsItem {
  ...
  fullContent: string;
  highlight: boolean;
  isSecret?: boolean; // Flag opzionale per stile speciale
}

// --- DATI DELLE NEWS (CONTENUTO) ---

// 1. News Segreta (EDEN)
const secretNews: NewsItem = {
  id: 999,
  date: "24/12/2001",
  month: "Dicembre 2001",
...
const News = () => {
  // Logica aggiornata: Hook per leggere i parametri URL (es. ?article=999)
  ...
  // Logica aggiornata: Fusione condizionale degli array
  const allNewsItems = [
    // Se la variabile globale è true, aggiunge la news segreta in cima
    ...(edenTried ? [secretNews] : []),
    ...baseNewsItems
  ];

  // Logica aggiornata: Effetto per aprire automaticamente una news dall'URL
  useEffect(() => {
    ...
    if (articleParam) {
      ...
      if (newsToOpen) {
        ...
        // Opzionale: scrolla se necessario, ma il dialog è modale quindi basta aprirlo
      }
    }
  }, [searchParams, edenTried]); // Riesegue se cambia l'URL o se si sbloccano nuove news
  ...
  const filteredNews = allNewsItems.filter((news) => {
    // La news segreta (URGENTE) appare sempre se si è su "Tutti", altrimenti segue il filtro
                ...
                paginatedNews.map((news) => (
                  <article
                    ...
                    className={`retro-panel ${
                      news.isSecret 
                        ? "border-destructive/70 bg-destructive/5" // Stile speciale per la news segreta
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <h2 className={`font-bold text-[12px] mb-2 ${news.isSecret ? "text-destructive" : "text-primary"}`}>
                          {news.highlight && <span className={`${news.isSecret ? "" : "text-destructive"} blink mr-1`}>
